        ...
        with torch.no_grad():
            ...
            if distributed:
                ...
                if env_params["rank"] == 0:
                    ...
                    # f = open(f"{folder_path}/experiments/expert_count_val.txt", "a")
                    # f.write(f"validation expert count, layer {layer_n}: \n")
                    # f.close()
                    # f = open(f"{folder_path}/experiments/expert_count_test.txt", "a")
                    # f.write(f"test expert count, layer {layer_n}: \n")
                    # f.close()
                    # np.savetxt(f"{folder_path}/experiments/expert_count_val.txt", expert_count_all_val.cpu().numpy())
                    # np.savetxt(f"{folder_path}/experiments/expert_count_test.txt", expert_count_all_test.cpu().numpy())
                else:
                    return
